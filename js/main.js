...
const TOPICS = [
  ...
  { 
    ...
    subtopics: [
      ...
      { id: 'rule-of-five', title: '3. Rule of Five', file: 'topics/02c-rule-of-five.md' },
      { id: 'inheritance', title: '4. Inheritance', file: 'topics/02d-inheritance.md' },
      { id: 'polymorphism', title: '5. Polymorphism', file: 'topics/02e-polymorphism.md' }
    ]
  },
...
function postProcessMarkdown(container) {
  ...
  // Process GitHub-style Alerts in Blockquotes
  const blockquotes = container.querySelectorAll('blockquote');
  blockquotes.forEach(bq => {
    const firstP = bq.querySelector('p');
    if (firstP) {
      const html = firstP.innerHTML;
      const match = html.match(/^\[!(NOTE|TIP|IMPORTANT|WARNING|CAUTION)\](<br>|\s*)([\s\S]*)/i);
      if (match) {
        const type = match[1].toLowerCase();
        const content = match[3];
        bq.className = `alert alert-${type}`;
        firstP.innerHTML = `<span class="alert-title">
