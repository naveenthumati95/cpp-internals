...
This chapter introduces the core ideas behind memory management, pointers, references, and the building blocks of modern C++ abstractions such as functors and lambdas.
...
- Functors and lambdas (coming soon)
...
```
check what happen if we call delete two times on array ??


##References##
A reference (T&) is an alias (another name) for an existing variable. It does not create a new object or allocate new memory; instead, it refers to the same memory location as the original variable. Any change made through the reference directly affects the original variable.
Impt-
1.A pointer has its own memory address and size on the stack, reference shares the
same memory address (with the original variable).
2.• The compiler can internally implement references as pointers, but treats them in a
very different way.
3.References allow better optimization because they must be initialized, cannot be null, and cannot be reseated to refer to another object. These guarantees let the compiler often eliminate the hidden pointer, replace the reference with the original variable, keep values in registers longer.For ex-
```cpp
int a = 10;
int& r = a;
...
- The original variable and the function parameter have different memory locations.
- Changes affect only the copy, not the original.
- For large objects, this can be expensive because copying takes time and memory.
...
- This is often faster for large objects because no copy is made.

> Practice these examples on your own to reinforce the difference between pointers and references.
